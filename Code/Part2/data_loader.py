...
class DataLoader(object):
    ...
    def data_augmentation(self, im, intrinsics, out_h, out_w):
        ...
        def random_coloring(im):
            ...
            im_aug  *= color_image
            # print("mic check",tf.shape(im))
            # exit()
            # Add gaussian noise.
            random_gaussian = tf.random_normal(shape=tf.shape(im),mean=0.0, stddev=0.01, dtype=tf.float32)
            im_aug += random_gaussian
    ...
    def batch_unpack_image_sequence(self, image_seq, img_height, img_width, num_source):
        # Assuming the center image is the target frame
        print(image_seq, img_height, img_width, num_source)
	exit()
        tgt_start_idx = int(img_width * (num_source//2))
        tgt_image = tf.slice(image_seq, 
                             [0, 0, tgt_start_idx, 0], 
